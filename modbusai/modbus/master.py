...
class RtuMaster:
    ...
    def execute(self, req: Request) -> ExchangeRecord:
        ...
        try:
            ...
            return ExchangeRecord(seq, timestamp, req, settings, tx, None, ExchangeStatus.TRANSPORT_ERROR, None,
        ...
        if rx is None:
            return ExchangeRecord(seq, timestamp, req, settings, tx, None, ExchangeStatus.TIMEOUT, None,
        ...
        try:
            ...
            return ExchangeRecord(seq, timestamp, req, settings, tx, rx, ExchangeStatus.CRC_ERROR, response_time_ms,
                                  error_message=str(exc))
        except ModbusException as exc:
            return ExchangeRecord(seq, timestamp, req, settings, tx, rx, ExchangeStatus.MODBUS_EXCEPTION,
                                  response_time_ms, exception_code=exc.code, error_message=str(exc))
        except BadResponse as exc:
            return ExchangeRecord(seq, timestamp, req, settings, tx, rx, ExchangeStatus.BAD_RESPONSE, response_time_ms,
                                  error_message=str(exc))
